...
Voli loads themes from:

- Built-in: `dark`, `light`
- Per-agent: `~/.voli/agents/<name>/themes/*.json`
- Shared: `~/.voli/agent/themes/*.json`
...
On first run, Voli detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.voli/agent/themes
vim ~/.voli/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, Voli reloads it automatically for immediate visual feedback.
...
Voli uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Voli falls back to the nearest approximation.
