...
- [The package.json `voli` Manifest](#the-packagejson-voli-manifest)
...
A plugin is a directory (or published package) with a `package.json` whose `"voli"` field names the contribution files. Each listed path is a normal source module loaded by the agent's resource loader at launch:

- **integrations** — transport modules registered via `voli.registerIntegration` (see [integrations.md](./integrations.md)).
...
The agent's discovery dirs (`~/.voli/agents/<name>/extensions/`, `integrations/`, `skills/`, etc.) are for hand-placed local resources. Installed plugins are **not** unpacked into those dirs — they stay in `.plugins/` and are resolved from there.

## The package.json `voli` Manifest

The plugin manager reads `package.json` and parses exactly the `"voli"` object. Every key is an array of paths (relative to the package root) or glob patterns:
...
No other keys under `"voli"` are read. (There is no gallery/preview metadata; voli has no package registry of its own — see [Publishing](#publishing).)
...
```json
{
  "name": "voli-integration-telegram",
  "type": "module",
  "voli": {
...
- If no `"voli"` manifest is present, the manager falls back to convention directories — `extensions/`, `integrations/`, `skills/`, `prompts/`, `themes/` — and auto-discovers files there. A bare file or a manifest-less directory with no convention dirs is treated as a single extension.
...
### Why peerDependencies on `@opsyhq/voli`

Contribution modules import host types and APIs from `@opsyhq/voli` (`IntegrationsAPI`, `ExtensionFactory`, etc.). The host process *provides* that package at runtime, so the plugin must not bundle its own copy. Declare it as a peer with a `"*"` range:

```json
{ "peerDependencies": { "@opsyhq/voli": "*" } }
```
...
```
my-plugin/
├── package.json        # name, type: "module", "voli" manifest, deps, peerDependencies
...
2. **Declare them in the manifest.** List each file under the matching `"voli"` key (above).
3. **Set `"type": "module"`** so `.ts`/`.js` modules load as ESM.
4. **Put runtime deps in `dependencies`** and **`@opsyhq/voli` in `peerDependencies`** with `"*"`.
...
A plugin is shared as an ordinary package. Voli installs from three source kinds and nothing else — there is no voli-hosted registry or installer. Pick the distribution that matches the source scheme you want users to install with:

| Distribution     | How users install                                         |
|------------------|-----------------------------------------------------------|
| npm registry     | `npm publish`, then `voli <agent> plugins install npm:<name>` |
...
Voli does not run `npm publish` for you; it only *consumes* one of these three source forms.
...
```bash
voli <agent> plugins install npm:@scope/pkg              # npm: registry package
voli <agent> plugins install npm:@scope/pkg@1.2.3        # pinned version
voli <agent> plugins install git:github.com/user/repo    # git: shorthand
voli <agent> plugins install git:github.com/user/repo@v1 # pinned ref
voli <agent> plugins install ./path/to/plugin            # local path
```
...
```bash
voli <agent> plugins install ./packages/voli/plugins/telegram   # installs, then auto-onboards if on a TTY
voli <agent> plugins configure ./packages/voli/plugins/telegram # re-run guided setup on demand
```

- **On install, on an interactive terminal (TTY):** onboarding runs immediately. Each onboarded service prints one result: `connected` (then a hint to run `voli <agent>` to use it), `cancelled` (you dismissed a prompt), `not-found` (the integration is not installed for the agent), `no-onboard` (it declares no guided setup), or an `error` with its message. Any non-`connected`/`cancelled` result makes the command exit non-zero.
- **On install, non-interactive (no TTY):** install completes but skips setup and points you at `voli <agent> plugins configure <source>` to finish later.
...
{
  "plugins": [
    "npm:voli-integration-scheduler",
    {
...
The shipped Telegram plugin (`packages/voli/plugins/telegram/`) is the canonical dual-half plugin. Its layout:

```
telegram/
├── package.json        # "voli": { integrations: ["./index.ts"], extensions: ["./telegram-chat.ts"] }
...
└── telegram-chat.ts    # mapping extension: routes each message into a per-chat Voli session,
...
```bash
voli my-agent plugins install ./packages/voli/plugins/telegram
# -> copies the package into ~/.voli/agents/my-agent/.plugins/local/telegram-<hash>/
# -> records "plugins": ["packages/voli/plugins/telegram"] in agent.json (agent-relative)
# -> runs onboard: prompts for the bot token, writes the account to integrations.json
```

Both `index.ts` and `telegram-chat.ts` resolve from that one copy. The transport starts; once the account is configured, the mapping extension activates and bidirectional chat is live. To package it for others, publish the same directory to npm (`voli-integration-telegram`) or a git repo and have them install with `npm:` / `git:` instead of the local path.

The shipped scheduler plugin (`packages/voli/plugins/scheduler/`) has the identical shape — `"integrations": ["./index.ts"]`, `"extensions": ["./scheduler-chat.ts"]`, one runtime dep (`croner`), and the same `@opsyhq/voli` peer — confirming the dual-half pattern is the convention, not Telegram-specific. The only manifest differences are the package name, the dependency, and the extension filename.
