...
Extensions are TypeScript modules that extend Voli's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in an agent's `~/.voli/agents/<name>/extensions/` for auto-discovery, or list extra paths under `extensions` in `settings.json`. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `voli.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `voli.registerCommand()`
...
> **Note:** The extension factory's first argument is named `voli` throughout this document. That name is just a convention for the extension API object — call it whatever you like. (The package.json manifest key used to declare extensions is `"voli"`; that key name is fixed and unrelated to the argument name.)
>
> Every event handler, command, shortcut, and custom-tool `execute` receives a context bag as its last argument: `ctx: ExtensionContext`, where `ExtensionContext = { session, ui, mode }`. Examples destructure it as `{ session, ui, mode }`. `session` is the live session this invocation acts on (carrying `sessionManager`, `model`, `sendMessage`, `compact`, etc.); `ui` is that session's presentation channel; `mode` is the current run mode. Agent-global capabilities (cwd, environments, model registry, session discovery, integrations, reload, shutdown) live on `voli`.
...
Create `~/.voli/agents/<name>/extensions/my-extension.ts`:

```typescript
import type { ExtensionAPI } from "@opsyhq/voli";
import { Type } from "typebox";

export default function (voli: ExtensionAPI) {
  // React to events
  voli.on("session_start", async (_event, { ui }) => {
    ui.notify("Extension loaded!", "info");
  });

  voli.on("tool_call", async (event, { ui }) => {
...
| Location | Scope |
|----------|-------|
| `~/.voli/agents/<name>/extensions/*.ts` | The agent (all sessions) |
| `~/.voli/agents/<name>/extensions/*/index.ts` | The agent (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `@opsyhq/voli` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
...
export default function (voli: ExtensionAPI) {
  // Subscribe to events
  voli.on("event_name", async (event, { ui }) => {
  ...
  voli.registerTool({ ... });
  voli.registerCommand("name", { ... });
  voli.registerShortcut("ctrl+x", { ... });
  voli.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, Voli awaits it before continuing startup. That means async initialization completes before `session_start`.
...
export default async function (voli: ExtensionAPI) {
...
The diagram above is the order *events* fire. Within a single event, multiple handlers can be registered — across extensions, and within one extension. Voli always invokes them in the same nesting:
...
So "extension load order" decides which extension's handlers run first, and registration order (the order you called `voli.on(...)` inside that extension's factory) decides ordering among handlers from the same extension.
...
You cannot control cross-extension ordering from inside an extension; it follows the order extensions are discovered/loaded. Within your own extension, order your `voli.on(...)` calls if one handler must observe another's mutation first.
...
```typescript
voli.on("session_start", async (event, { session, ui }) => {
...
After a successful switch or new-session action, Voli emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
voli.on("session_before_compact", async (event, ctx) => {
...
```typescript
voli.on("session_shutdown", async (event, ctx) => {
...
```typescript
voli.on("before_agent_start", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data Voli uses to build the system prompt (type `BuildSystemPromptOptions`). This lets you inspect what Voli has loaded — the agent config, frozen SOUL/MEMORY/USER snapshots, discovered skills, the active tool names, and any appended system-prompt text — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.
...
```typescript
voli.on("agent_start", async (_event, ctx) => {});

voli.on("agent_end", async (event, ctx) => {
...
```typescript
voli.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

voli.on("turn_end", async (event, ctx) => {
...
```typescript
voli.on("message_start", async (event, ctx) => {
  // event.message
});

voli.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

voli.on("message_end", async (event, ctx) => {
...
```typescript
voli.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

voli.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

voli.on("tool_execution_end", async (event, ctx) => {
...
```typescript
voli.on("context", async (event, ctx) => {
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.session.getSystemPrompt()`, which reports Voli's system prompt string rather than the final serialized provider payload.

```typescript
voli.on("before_provider_request", (event, ctx) => {
...
```typescript
voli.on("model_select", async (event, { ui }) => {
...
```typescript
voli.on("thinking_level_select", async (event, { ui }) => {
...
Before `tool_call` runs, Voli waits for previously emitted Agent events to finish draining. This means `ctx.session.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@opsyhq/voli";

voli.on("tool_call", async (event, ctx) => {
...
```typescript
import { isBashToolResult } from "@opsyhq/voli";

voli.on("tool_result", async (event, { session }) => {
...
```typescript
import { createHostEnvironment } from "@opsyhq/voli";

voli.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap Voli's host environment to rewrite commands
...
Outside a handler (for example, in an integration `.on(...)` callback registered at load time), there is no `ctx` in scope. Reach a session through the agent-global discovery methods on `voli` — `voli.getSession(id)`, `voli.openSession(id)`, `voli.createSession()` — described in [ExtensionAPI Methods](#extensionapi-methods).
...
The current model, or `undefined`. For the model registry and API keys, use `voli.modelRegistry` (see [ExtensionAPI Methods](#extensionapi-methods)).
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while Voli is idle.
...
Returns Voli's current system prompt string.
...
```typescript
voli.on("before_agent_start", (event, { session }) => {
...
Returns the base inputs Voli currently uses to build the system prompt.
...
```typescript
voli.on("session_start", async (_event, { session }) => {
...
Read or merge the session's folded tags — a durable, append-only key/value binding an extension owns (for example, to an external chat). Core never interprets the keys. Query across sessions with `voli.findSessions(...)`.
...
```typescript
const model = voli.modelRegistry.find("anthropic", "claude-sonnet-4-5");
if (model) {
...
```typescript
voli.registerCommand("reload-runtime", {
...
`voli.reload()` runs the same flow at the agent-global level.
...
Methods are split across two objects. The agent-global ones live on `voli` (the extension factory argument): registration, provider management, integrations, the shared event bus, and session discovery/creation.

Per-session actions (`sendMessage`, `sendUserMessage`, `appendEntry`, `setSessionName`, `setActiveTools`, `setModel`, `setThinkingLevel`, etc.) live on `ctx.session` instead — destructure `{ session }` from a handler's context, or reach a session through `voli.getSession(id)` / `voli.openSession(id)` outside a handler. The `ctx.session` members are documented in [ctx.session members](#ctxsession-members); the `voli.*` methods are documented here.

### voli.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### voli.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`voli.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `ctx.session.getAllTools()` and are callable by the LLM without `/reload`.
...
### voli.registerCommand(name, options)
...
If multiple extensions register the same command name, Voli keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.

```typescript
voli.registerCommand("stats", {
...
### voli.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for messages with your `customType`. See [Custom UI](#custom-ui).

### voli.registerShortcut(shortcut, options)

Register a keyboard shortcut.

```typescript
voli.registerShortcut("ctrl+shift+p", {
...
### voli.registerFlag(name, options) / voli.getFlag(name)

Register a CLI flag and read its value.

```typescript
voli.registerFlag("plan", {
...
// Check value
if (voli.getFlag("plan")) {
  // Plan mode enabled
}
```

### voli.cwd / voli.environments / voli.modelRegistry

Agent-global, read-only.

- `voli.cwd` - the agent's home directory, where its files and the file/shell tools operate.
- `voli.environments` - the full run-target map (type `AgentEnvironments`), including the unconfined `host` target. Reach a specific target via `voli.environments.targets[...]`.
- `voli.modelRegistry` - model registry for API key resolution and provider registration.

### voli.getSession(id) / voli.openSession(id) / voli.createSession(options?) / voli.listSessions() / voli.findSessions(filter)
...
- `voli.getSession(id)` returns a resident (in-memory) `Session` by id, or `undefined` when it is not currently resident. Find-only — never creates or loads.
- `voli.openSession(id)` rehydrates a stored session by id into the resident set (or returns it if already resident), resolving to a `Session`.
- `voli.createSession(options?)` starts a fresh stored session and makes it resident. Additive — other sessions stay live. Accepts the same `NewSessionOptions` as `session.newSession()`.
- `voli.listSessions()` returns the stored sessions for this agent (newest first) as `SessionInfo[]`.
- `voli.findSessions(filter)` locates stored sessions whose folded tags subset-match `filter`, each with `tags` populated — for example, the session another extension bound to an external conversation via `session.setTags(...)`.

```typescript
// Reach (or create) the session bound to an external chat, from a background callback.
const [match] = await voli.findSessions({ "telegram:chat": String(chatId) });
const session = match ? await voli.openSession(match.id) : await voli.createSession();
...
### voli.reload() / voli.shutdown()

- `voli.reload()` runs the same reload flow as `/reload` (see [session.reload()](#sessionreload)). Use this at the agent-global level.
- `voli.shutdown()` requests a graceful shutdown of Voli.
...
### voli.registerProvider(name, config) / voli.unregisterProvider(name)

Register, override, or remove a model provider. See the inline examples in the `@opsyhq/voli` type definitions for `ProviderConfig` (custom models, baseUrl overrides, and OAuth). To make a registered provider's models usable, log in with `/login` (subscription/OAuth) or supply the provider's API key; OAuth providers registered via `config.oauth` surface in `/login`.

### voli.getIntegration(name, account?)
...
```typescript
voli.events.on("my:event", (data) => { ... });
voli.events.emit("my:event", { ... });
```
...
```typescript
voli.on("session_start", (_event, { session }) => {
  for (const entry of session.sessionManager.getBranch()) {
...
Register tools the LLM can call via `voli.registerTool()`. Tools appear in the system prompt and can have custom rendering.
...
Pass the real target file path to `withFileMutationQueue()`, not the raw user argument. Resolve it to an absolute path first, relative to `voli.cwd` or your tool's working directory. For existing files, the helper canonicalizes through `realpath()`, so symlink aliases for the same file share one queue. For new files, it falls back to the resolved absolute path because there is nothing to `realpath()` yet.

Queue the entire mutation window on that target path. That includes read-modify-write logic, not just the final write.

```typescript
import { withFileMutationQueue } from "@opsyhq/voli";
import { mkdir, readFile, writeFile } from "node:fs/promises";
import { dirname, resolve } from "node:path";

async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
  const absolutePath = resolve(voli.cwd, params.path);
...
**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when Voli resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
```typescript
import { createBashTool, createHostEnvironment } from "@opsyhq/voli";

const env = createHostEnvironment(voli.cwd);
...
export default function (voli: ExtensionAPI) {
  ...
  voli.registerTool({ name: "db_connect", ... });
  voli.registerTool({ name: "db_query", ... });
  voli.registerTool({ name: "db_close", ... });

  voli.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
```typescript
voli.on("session_start", (_event, { ui }) => {
  ui.addAutocompleteProvider((current) => ({
    triggerCharacters: ["#"],
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@opsyhq/voli";
...
voli.registerMessageRenderer("my-extension", (message, options, theme) => {
...
```typescript
import { highlightCode, getLanguageFromPath } from "@opsyhq/voli";
...
These examples combine the pieces above against the real `@opsyhq/voli` surface.

### Bind a session to an external conversation

Use folded tags plus `voli.findSessions` / `openSession` / `createSession` to route an external chat to a stable session, with no live `ctx` in scope.

```typescript
import type { ExtensionAPI } from "@opsyhq/voli";

export default function (voli: ExtensionAPI) {
  const telegram = voli.getIntegration("telegram", "default");

  telegram.on("message", async (msg) => {
    const key = String(msg.chatId);
    const [match] = await voli.findSessions({ "telegram:chat": key });

    const session = match
      ? await voli.openSession(match.id)
      : await voli.createSession({
...
export default function (voli: ExtensionAPI) {
  voli.on("tool_call", async (event, { ui, mode }) => {
