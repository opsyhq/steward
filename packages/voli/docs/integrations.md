...
> **Note:** The integration factory's first argument is named `voli` throughout this document — by convention, the same as the extension factory argument. It is the `IntegrationsAPI` object; call it whatever you like. The package.json manifest key that declares integrations is `"voli"`; that key name is fixed and unrelated to the argument name.
>
> An integration has two halves. The **producer/transport** half (`run`, `actions`, `events`, `onboard`) is the integration module itself, registered via `voli.registerIntegration`. The **mapping** half is an ordinary extension that consumes the integration via `voli.getIntegration(...)`. The two ship together as one package (see [The dual-half package](#the-dual-half-package)).
...
```typescript
import type { IntegrationsAPI } from "@opsyhq/voli";
import { Type } from "typebox";

export default function (voli: IntegrationsAPI) {
...
export default function (voli: ExtensionAPI) {
  const ticker = voli.getIntegration("ticker", "default");
  ticker.on("tick", async (data) => {
    const { at } = data as { at: number };
    const tag = { "ticker:default": "1" };
    const [match] = await voli.findSessions(tag);
    const session = match
...
```bash
voli <agent> plugins install ./path/to/ticker
```
...
| Location | Scope |
|----------|-------|
| `~/.voli/agents/<name>/integrations/*.ts` | The agent (all sessions) |
| `~/.voli/agents/<name>/integrations/*/index.ts` | The agent (subdirectory) |

Plugins installed with `voli <agent> plugins install <source>` are resolved into this folder by the package manager. Configured account credentials live separately, in the per-agent `integrations.json`:
...
| Package | Purpose |
|---------|---------|
| `@opsyhq/voli` | Integration types (`IntegrationsAPI`, `IntegrationConfig`, `IntegrationOnboardContext`, `IntegrationRunContext`, `IntegrationActionContext`, `KeyValueStore`, `IntegrationHandle`) |
| `typebox` | Schemas (`Type`) for `account`, `events`, and action `parameters` |

The integration types are host-provided. A shipped integration package declares `@opsyhq/voli` as a **peerDependency**, not a dependency — the host supplies it at load time. The package brings its own transport dependencies (the Telegram plugin bundles `grammy` + `@grammyjs/runner`; the scheduler bundles `croner`).
...
An integration module default-exports a factory receiving `IntegrationsAPI`. Inside, call `voli.registerIntegration(config)` once per service. The factory may be synchronous or asynchronous.

```typescript
import type { IntegrationsAPI } from "@opsyhq/voli";

export type IntegrationFactory = (voli: IntegrationsAPI) => void | Promise<void>;
```
...
`run` is the one genuinely new concept in voli, which is otherwise pull/request-response. It opens a connection or loop and pushes inbound items out as events. The host calls `run` once per `(service, account)` after the account validates.
...
`ctx.store` is a string-keyed `KeyValueStore` for machine-written runtime state, scoped to one service and backed by `~/.voli/agents/<name>/store/<service>.json`. It is process-scoped and survives `/reload`. Use it where an integration keeps state it owns (the scheduler keeps its jobs here).
...
The mapping half is an ordinary extension. It reaches the transport through `voli.getIntegration(name, account?)`, which returns an `IntegrationHandle`. `account` defaults to `"default"`.
...
export default function (voli: ExtensionAPI) {
  const tg = voli.getIntegration("telegram", "default"); // throws if telegram.default not configured
...
A shipped integration is one package declaring both halves under its package.json `"voli"` key:
...
Why the split is load-bearing: the transport (`index.ts`) never touches sessions or the agent; the extension (`*-chat.ts`) owns the mapping between the external thread and a Voli session. Swapping the mapping is an extension change; the transport is untouched.
...
```bash
voli <agent> plugins install <source>      # install + auto-onboard (on a TTY)
voli <agent> plugins configure <source>    # re-run guided setup (requires a TTY)
voli <agent> plugins list                  # list installed plugins + contributed integrations
voli <agent> plugins remove <source>       # remove the plugin
voli <agent> plugins update [source]       # update one or all plugins
```
...
| Form | Example |
|------|---------|
| local | `voli <agent> plugins install ./plugins/telegram` |
| npm | `voli <agent> plugins install npm:@scope/pkg` |
| git | `voli <agent> plugins install git:github.com/user/repo` |
...
The Telegram plugin is the canonical dual-half integration. The transport (`index.ts`) long-polls grammY, holds the bot token, and emits a `message` event per inbound message. It exposes `sendMessage`, `sendChatAction`, and `setCommands` actions. The mapping (`telegram-chat.ts`) routes each message into a per-chat Voli session and ships the reply back.

The transport — events, an action, onboarding, and the long-poll producer:

```typescript
import { run } from "@grammyjs/runner";
import type { IntegrationOnboardContext, IntegrationsAPI } from "@opsyhq/voli";
...
export default function (voli: ExtensionAPI) {
  const tg = voli.getIntegration("telegram", "default");
  ...
  tg.on("message", async (data) => {
    ...
    const [match] = await voli.findSessions(chatTag);
    const session = match
      ? await voli.openSession(match.id)
      : await voli.createSession({
          setup: async (sessionManager) => {
  ...
  voli.on("agent_end", async ({ messages }, ctx) => {
...
import type { IntegrationOnboardContext, IntegrationsAPI, KeyValueStore } from "@opsyhq/voli";
...
export default function (voli: ExtensionAPI) {
  const sched = voli.getIntegration("scheduler", "default");
  ...
  sched.on("due", async (data) => {
    ...
    const [match] = await voli.findSessions(job.originTags ?? {});
    const session = match
      ? await voli.openSession(match.id)
      : await voli.createSession({
          setup: async (sessionManager) => {
