...
Voli is a persistent, purposeful agent that runs in your terminal. Unlike a
chat session, a Voli agent is created *for* something — a purpose stated by
...
```bash
npm install -g @opsyhq/voli
```

Voli reuses credentials that another agent CLI may have already set up under
`~/.voli/agent/` (API keys and OAuth tokens). To set a key explicitly, export
it before starting:
...
```
voli new <name> [--model provider/id]      Create an agent, then start its birth conversation
voli list                                  List agents
voli delete <name>                          Delete an agent (type-the-name confirm)
voli <name> [message] [--new] [--print]    Talk to an agent
```
...
Running `voli <name>` drops you into an interactive terminal UI (TUI). By
...
```bash
voli calories "log: two eggs and toast" --print
```
...
[Agent homes](#agent-homes)). When you start an agent without `--new`, Voli
...
Voli keeps each agent's state in its own home directory so deleting an agent
also cleans up everything attached to it.

| Path | Contents |
|------|----------|
| `~/.voli/agents/<name>/` | Per-agent home |
| `~/.voli/agents/<name>/agent.json` | Identity config (name, purpose, deploy state) |
| `~/.voli/agents/<name>/SOUL.md` | Curated identity |
| `~/.voli/agents/<name>/MEMORY.md` | Curated durable memory |
| `~/.voli/agents/<name>/USER.md` | Curated facts about the human |
| `~/.voli/agents/<name>/sessions/` | Append-only session tree |
| `~/.voli/agents/<name>/workspace/` | The agent's owned working directory |
| `~/.voli/agents/<name>/{extensions,skills,prompts,themes}/` | Per-agent customizations |
| `~/.voli/agent/` | Shared credentials (`auth.json`) and default model settings |

Override the root with `VOLI_HOME` (defaults to `~/.voli`).

## SDK usage

Voli exposes its agent builder programmatically from `@opsyhq/voli`. The
...
(`~/.voli/agent/auth.json`) populated by `/login`, then OAuth tokens, then
