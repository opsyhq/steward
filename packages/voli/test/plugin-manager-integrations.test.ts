 ...
 *  - a dual-half plugin (one manifest declaring both `voli.integrations` and
 *    `voli.extensions`) resolves BOTH halves from a single managed copy under
...
beforeEach(() => {
	previousOfflineEnv = process.env.VOLI_OFFLINE;
	...
	process.env.VOLI_HOME = tempDir;
	// Isolate the shared defaults to an (empty) temp dir so the merge can't pick up the
	// real user's ~/.voli/agent/settings.json.
	process.env.VOLI_SHARED_DIR = join(tempDir, "shared");
...
afterEach(() => {
	...
	if (previousOfflineEnv === undefined) {
		...
		process.env.VOLI_OFFLINE = previousOfflineEnv;
	}
...
function writeDualHalfPackage(dir: string): { integrationPath: string; extensionPath: string } {
	...
	writeFileSync(
		...
		JSON.stringify({
			name: "telegram-chat",
			voli: { integrations: ["./index.ts"], extensions: ["./telegram-chat.ts"] },
		}),
	);
