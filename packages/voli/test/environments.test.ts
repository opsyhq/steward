/**
 * The Environment seam — backend selection (VOLI_SANDBOX override + platform
...
describe("createEnvironments target selection", () => {
	...
	it("honors VOLI_SANDBOX=local-os without consulting platform support", async () => {
	...
	it("collapses to a single silent host target on VOLI_SANDBOX=host, never touching srt", async () => {
	...
	it("selects the docker backend on VOLI_SANDBOX=docker", async () => {
...
describe("local write-jail", () => {
	...
	beforeEach(() => {
		jail = mkdtempSync(join(tmpdir(), "voli-jail-"));
		outside = mkdtempSync(join(tmpdir(), "voli-outside-"));
	});
