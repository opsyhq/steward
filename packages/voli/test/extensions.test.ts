 ...
 *  4. createConversation() invalidates the superseded runner (a captured voli goes stale).
 */
...
export default function testExtension(pi) {
	...
	pi.on("session_start", (event) => {
		const dir = process.env.VOLI_TEST_MARKER_DIR;
...
// Registers a `/cap` command whose handler closes over the agent-global `voli`; calling a voli
...
export default function staleExtension(pi) {
	pi.registerCommand("cap", {
		description: "Calls a voli method; throws once the runner is superseded.",
		async handler() {
...
// stored-session count (off `voli.listSessions()`) to the marker dir.
...
export default function facadeExtension(pi) {
	pi.registerCommand("facade", {
		description: "Records the acting session id + session count via ctx.session/voli.listSessions().",
...
beforeEach(() => {
	home = mkdtempSync(join(tmpdir(), "voli-ext-home-"));
	sharedDir = mkdtempSync(join(tmpdir(), "voli-ext-shared-"));
	markerDir = mkdtempSync(join(tmpdir(), "voli-ext-marker-"));
	process.env.VOLI_HOME = home;
	process.env.VOLI_SHARED_DIR = sharedDir;
	process.env.VOLI_TEST_MARKER_DIR = markerDir;
...
afterEach(async () => {
	...
	delete process.env.VOLI_TEST_MARKER_DIR;
...
describe("extension subsystem wiring", () => {
	...
	it("invalidates a captured voli on reload", async () => {
		...
		// The captured voli works before the reload.
		...
		// Any voli captured from the superseded runner now throws stale.
	...
	it("exposes ctx.session/voli.listSessions() so a command resolves the acting session and lists sessions", async () => {
		...
		// voli.listSessions() surfaced the stored session create() wrote eagerly.
