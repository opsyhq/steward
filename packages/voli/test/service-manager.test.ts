...
beforeEach(() => {
	home = mkdtempSync(join(tmpdir(), "voli-svc-"));
	process.env.VOLI_HOME = home;
	process.env.VOLI_SHARED_DIR = join(home, "shared");
});
...
describe("detectServiceManager", () => {
	it("honors the VOLI_SERVICE_MANAGER override", () => {
...
describe("buildLaunchAgentPlist", () => {
	const plist = buildLaunchAgentPlist({
		...
		environment: { VOLI_HOME: "/home/.voli", AMP: "a&b" },
	});
	...
	it("renders the program arguments and environment, XML-escaped", () => {
		...
		expect(plist).toContain("<key>VOLI_HOME</key>");
		expect(plist).toContain("<string>/home/.voli</string>");
...
describe("buildSystemdUnit", () => {
	const unit = buildSystemdUnit({
		description: "voli agent test",
		execStart: ["/usr/bin/node", "/opt/my cli.js", "daemon", AGENT],
		workingDirectory: "/home/agent",
		environment: { VOLI_HOME: "/home/.voli" },
	});

	it("renders an always-on user service", () => {
		expect(unit).toContain("Restart=always");
		expect(unit).toContain("WantedBy=default.target");
		expect(unit).toContain("Environment=VOLI_HOME=/home/.voli");
	});
	...
	it("names the unit per agent", () => {
		expect(systemdUnitName(AGENT)).toBe(`voli-${AGENT}.service`);
	});
});
