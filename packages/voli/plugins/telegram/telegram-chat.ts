 ...
 * this extension maps that transport onto a Voli session:
 *
 *   - inbound:  `telegram.on("message")` routes the text into this chat's own session by
 *               `voli.findSessions({ "telegram:chat": <id> })` → `openSession` the match (or
 *               `createSession` + tag a fresh one) → `session.sendUserMessage(text)`
 *   - outbound: `voli.on("agent_end")` reads the PRODUCING session's tag off
 ...
 *
 * Session binding via tags: each chat gets its OWN Voli session, bound by a `{ "telegram:chat":
 ...
 * extension with `voli.findSessions({ "telegram:chat": <id> })`.
 *
 * This file is declared under the package's `voli.extensions` and is resolved in place by the
 * package manager when the integration is onboarded
 * (`voli <agent> plugins configure telegram`); it activates on the next launch.
 */
...
export default function (voli: ExtensionAPI) {
	...
	async function handleCommand(chatId: number, text: string): Promise<void> {
		...
		switch (command) {
			...
			case "status": {
				const matches = await voli.findSessions({ "telegram:chat": String(chatId) });
				const name = (matches[0] && voli.getSession(matches[0].id)?.getSessionName()) ?? "(unnamed)";
