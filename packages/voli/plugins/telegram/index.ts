 ...
 * paired extension (`telegram-chat.ts`, declared under `voli.extensions`) maps those
 ...
 * ## Install + configure
 *
 *   voli <agent> plugins install ./plugins/telegram
 ...
 * and stores the raw token in `~/.voli/agents/<name>/integrations.json`:
 *
...
// @opsyhq/voli is a peerDependency, not a dependency.
...
const ONBOARD_GUIDE = [
	...
	"",
	"Voli verifies the token and stores it for this agent.",
].join("\n");
