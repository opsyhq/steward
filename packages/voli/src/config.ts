 ...
 * and every on-disk location is a `getXxxDir`/`getXxxPath` getter. Voli's
...
export const PACKAGE_NAME: string = pkg.name || "@opsyhq/voli";
export const APP_NAME: string = piConfigName || "voli";
export const APP_TITLE: string = APP_NAME;
export const CONFIG_DIR_NAME: string = pkg.piConfig?.configDir || ".voli";
...
// Override for the shared credential dir, e.g. VOLI_SHARED_DIR.
// Resolves the shared `~/.voli/agent/` holding auth.json/settings.json.
export const ENV_SHARED_AGENT_DIR = `${APP_NAME.toUpperCase()}_SHARED_DIR`;

// Override for a daemon's bearer token, e.g. VOLI_DAEMON_TOKEN. When set, the
...
// Override for the host the daemon binds, e.g. VOLI_DAEMON_HOST. Defaults to loopback
...
// Force the OS service backend, e.g. VOLI_SERVICE_MANAGER=none|launchd|systemd. Overrides
...
// Select the file/shell confinement backend, e.g. VOLI_SANDBOX=host|local-os|docker|auto (default `auto`).
...
// Skip host-escalation prompts: VOLI_BYPASS_PERMISSIONS=1 auto-approves every host command. The
...
// Image for the docker sandbox backend, e.g. VOLI_CONTAINER_IMAGE. Falls back to
// DEFAULT_CONTAINER_IMAGE when unset.
export const ENV_CONTAINER_IMAGE = `${APP_NAME.toUpperCase()}_CONTAINER_IMAGE`;

// Default image for the docker sandbox backend (VOLI_SANDBOX=docker): a published multi-arch image
// = debian:stable-slim + ripgrep/fd (grep/find run them inside the container). Built and pushed from
// docker/sandbox.Dockerfile. Override with VOLI_CONTAINER_IMAGE (used as-is, expected to carry rg+fd).
export const DEFAULT_CONTAINER_IMAGE = "ghcr.io/opsyhq/voli-sandbox:1";
...
/** User's custom themes dir, e.g. ~/.voli/agent/themes (shared agent dir). */
...
// =============================================================================
// Agent home paths (~/.voli/agents/<name>/)
// =============================================================================

/** Root config dir, e.g. ~/.voli (override with VOLI_HOME). */
...
/** Root dir holding all agents, e.g. ~/.voli/agents */
export function getAgentsRoot(): string {
	return join(getHomeDir(), "agents");
}

/** Per-agent home dir, e.g. ~/.voli/agents/<name> */
...
/** Path to an agent's per-integration runtime state dir, e.g. ~/.voli/agents/<name>/store */
export function getAgentStoreDir(name: string): string {
	return join(getAgentDir(name), "store");
}

/** Path to one integration's runtime state file, e.g. ~/.voli/agents/<name>/store/<service>.json */
...
/** Root dir for daemon log files (launchd/systemd stdout/stderr), e.g. $TMPDIR/voli-daemons. */
export function getDaemonRuntimeDir(): string {
	return join(tmpdir(), `${APP_NAME}-daemons`);
}

/** The host the daemon binds (VOLI_DAEMON_HOST), defaulting to loopback. */
export function getDaemonHost(): string {
	return process.env[ENV_DAEMON_HOST]?.trim() || "127.0.0.1";
}

/** The VOLI_DAEMON_TOKEN override, if set — callers fall back to the agent's persisted token. */
export function getDaemonToken(): string | undefined {
	return process.env[ENV_DAEMON_TOKEN]?.trim();
}

// =============================================================================
// Shared credential store (~/.voli/agent/)
// =============================================================================
//
// Voli reuses the credentials and default model in the shared `agent/` dir,
// so a Codex OAuth login (or any env/api key) works with no re-login. These
// resolve the singular shared `agent` dir, distinct from voli's own per-agent
// homes under `agents/<name>/` (plural).

/** Shared agent dir, e.g. ~/.voli/agent (override with VOLI_SHARED_DIR). */
