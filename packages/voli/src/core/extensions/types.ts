...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@opsyhq/voli` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@opsyhq/voli";
	 *
...
/** Options for `Session.newSession()` / `voli.createSession()`. */
 ...
 * to every event, command, shortcut, and custom-tool handler, and returned from `voli.getSession()`.
 *
...
export interface Session {
	 ...
	 * external chat). Core never interprets the keys; query across sessions via `voli.findSessions`.
	 */
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what voli loaded without re-discovering resources. */
...
/** Options accepted by `voli.registerCommand`. */
...
export interface ExtensionAPI {
	 ...
	 * `voli.environments.targets[...]` to reach a specific target.
	 */
	...
	/** Gracefully shut down voli and exit. */
	shutdown(): void;
	 ...
	 * voli.registerProvider("my-proxy", {
	 ...
	 * voli.registerProvider("anthropic", {
	 *   baseUrl: "https://proxy.example.com"
	 * });
	 *
	 * @example
	 * // Register provider with OAuth support
	 * voli.registerProvider("corporate-ai", {
	 ...
	 *
	 * @example
	 * voli.unregisterProvider("my-proxy");
	 */
	 ...
	 *   const [match] = await voli.findSessions({ "telegram:chat": String(msg.chatId) });
...
/** Configuration for registering a provider via voli.registerProvider(). */
...
export interface ProviderModelConfig {
	...
	reasoning: boolean;
	/** Maps voli thinking levels to provider/model-specific values; null marks a level unsupported. */
...
export type ExtensionFactory = (voli: ExtensionAPI) => void | Promise<void>;
...
export interface ExtensionRuntimeState {
	// Agent-global capabilities backing the `voli.*` methods. Throwing stubs during extension load;
	// the runtime overrides them (closures over the AgentRuntime) once resources are built — so accessing
	// them during load throws (mirrors the old `voli.agent` resolution).
	getSession: (id: string) => Session | undefined;
	openSession: (id: string) => Promise<Session>;
	createSession: (options?: NewSessionOptions) => Promise<Session>;
	listSessions: () => Promise<SessionInfo[]>;
