...
// avoiding a circular dependency. Extensions can import from @opsyhq/voli.
...
export const VIRTUAL_MODULES: Record<string, unknown> = {
	...
	// The host-package identity string is voli's own package name.
	"@opsyhq/voli": _bundledPiCodingAgent,
...
export function getAliases(): Record<string, string> {
	...
	_aliases = {
		// The host-package identity string is voli's own package name.
		"@opsyhq/voli": piCodingAgentEntry,
...
export function createExtensionRuntime(): ExtensionRuntime {
	...
	const runtime: ExtensionRuntime = {
		// Agent-global capabilities backing `voli.*` — throwing stubs until the runtime overrides
...
function readVoliManifest(packageJsonPath: string): VoliManifest | null {
	try {
		const content = fs.readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content);
		if (pkg.voli && typeof pkg.voli === "object") {
			return pkg.voli as VoliManifest;
		}
		return null;
	} catch {
		return null;
 ...
 *
 * Checks for:
 * 1. package.json with "voli.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
	// Check for package.json with "voli" field first
	const packageJsonPath = path.join(dir, "package.json");
	if (fs.existsSync(packageJsonPath)) {
		const manifest = readVoliManifest(packageJsonPath);
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "voli" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
	...
	for (const p of configuredPaths) {
		...
		if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
			// Check for package.json with voli manifest or index.ts
