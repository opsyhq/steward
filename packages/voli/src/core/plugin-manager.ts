...
// PathMetadata + SourceScope are owned by source-info.ts in voli (single source
...
function isOfflineModeEnabled(): boolean {
	const value = process.env.VOLI_OFFLINE;
...
// Voli is per-agent only: the sole non-temporary install scope is "user"
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "voli" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readPluginManifestFile(packageJsonPath: string): PluginManifest | null {
	try {
		...
		const pkg = JSON.parse(content) as { voli?: PluginManifest };
		return pkg.voli ?? null;
	} catch {
		return null;
	}
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "voli");
	}
...
export class DefaultPluginManager implements PluginManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension/integration packages run inside voli and resolve host APIs through loader
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "voli-extensions", private: true };
	...
	private getBaseDirForScope(_scope: SourceScope): string {
		// Voli is per-agent only: the agent home is the single install/resolve base.
	...
	private addAutoDiscoveredResources(
		...
		// dirs. There is no project-local (`.voli/`) or ancestor `.agents/`
		...
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "voli"),
			userMetadata,
