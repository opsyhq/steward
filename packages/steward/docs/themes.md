...
Steward loads themes from:

- Built-in: `dark`, `light`
- Per-agent: `~/.steward/agents/<name>/themes/*.json`
- Shared: `~/.steward/agent/themes/*.json`
...
On first run, Steward detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.steward/agent/themes
vim ~/.steward/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, Steward reloads it automatically for immediate visual feedback.
...
Steward uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Steward falls back to the nearest approximation.
