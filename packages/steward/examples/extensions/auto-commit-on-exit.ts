...
export default function (steward: ExtensionAPI) {
	steward.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await steward.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[Steward] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await steward.exec("git", ["add", "-A"]);
		const { code: commitCode } = await steward.exec("git", ["commit", "-m", commitMessage]);
