 ...
 *
 * Demonstrates steward.sendUserMessage() for sending user messages from extensions.
 * Unlike steward.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (steward: ExtensionAPI) {
	...
	steward.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			steward.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	steward.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				steward.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	steward.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				steward.sendUserMessage(args, { deliverAs: "followUp" });
	...
	steward.registerCommand("askwith", {
