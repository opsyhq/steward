...
async function resolveGitHubRepo(steward: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await steward.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (steward: ExtensionAPI): void {
	steward.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(steward, ctx.cwd);
		if (!resolvedRepo.ok) {
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				const result = await steward.exec(
					"gh",
					[
						"issue",
						"list",
