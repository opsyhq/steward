...
async function checkDirtyRepo(
	steward: ExtensionAPI,
	ctx: ExtensionContext,
	action: string,
): Promise<{ cancel: boolean } | undefined> {
	// Check for uncommitted changes
	const { stdout, code } = await steward.exec("git", ["status", "--porcelain"]);
...
export default function (steward: ExtensionAPI) {
	steward.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(steward, ctx, action);
	});

	steward.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(steward, ctx, "fork");
	});
}
