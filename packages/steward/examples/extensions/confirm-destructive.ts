...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@opsyhq/steward";

export default function (steward: ExtensionAPI) {
	steward.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
