 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit Steward cleanly.
 */
...
export default function (steward: ExtensionAPI) {
	// Register a /quit command that cleanly exits Steward
	steward.registerCommand("quit", {
		description: "Exit Steward cleanly",
	...
	steward.registerTool({
		...
		description: "Complete a task and exit Steward",
	...
	steward.registerTool({
		...
		description: "Deploy the application and exit Steward",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await steward.exec("npm", ["run", "deploy", params.environment], { signal });
