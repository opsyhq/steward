 ...
 *
 * Runs Steward's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 *
 * Setup:
 *   cd packages/steward/examples/extensions/gondolin
 *   npm install --ignore-scripts
 *
 * Usage:
 *   cd /path/to/project
 *   steward -e /path/to/steward/packages/steward/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (steward: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `steward ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	steward.registerCommand("gondolin", {
	...
	steward.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
