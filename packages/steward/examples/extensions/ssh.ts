 ...
 *
 * Usage:
 *   steward -e ./ssh.ts --ssh user@host
 *   steward -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (steward: ExtensionAPI) {
	steward.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	steward.on("session_start", async (_event, ctx) => {
		...
		const arg = steward.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
