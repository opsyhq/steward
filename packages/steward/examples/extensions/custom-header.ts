 ...
 * (logo + keybinding hints) with a custom component showing the Steward mascot.
 */

import type { ExtensionAPI, Theme } from "@opsyhq/steward";
import { VERSION } from "@opsyhq/steward";

// --- STEWARD MASCOT ---
// Based on pi_mascot.ts - the Steward agent character
...
export default function (steward: ExtensionAPI) {
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						...
						const subtitle = `${theme.fg("muted", "   Steward")}${theme.fg("dim", ` v${VERSION}`)}`;
	...
	steward.registerCommand("builtin-header", {
