 ...
 * working indicator shown while Steward is streaming a response.
 *
 * Usage:
 *   steward --extension examples/extensions/working-indicator.ts
 *
 * Commands:
 ...
 *   /working-indicator reset     Restore Steward's default spinner
 */

import type { ExtensionAPI, ExtensionContext, WorkingIndicatorOptions } from "@opsyhq/steward";
...
function describeMode(mode: WorkingIndicatorMode): string {
	switch (mode) {
		...
		case "default":
			return "Steward default spinner";
	}
}
...
export default function (steward: ExtensionAPI) {
	...
	steward.registerCommand("working-indicator", {
