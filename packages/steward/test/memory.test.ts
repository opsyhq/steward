...
import { createSelfUpdateTool, type SelfUpdateToolDetails } from "../src/core/tools/memory.ts";
...
function run(op: MemoryOp): Promise<AgentToolResult<SelfUpdateToolDetails>> {
	return createSelfUpdateTool("scribe").execute("call-1", op);
}

function firstText(result: AgentToolResult<SelfUpdateToolDetails>): string {
	const block = result.content[0];
	return block.type === "text" ? block.text : "";
}

describe("self_update tool", () => {
	it("is exposed under the self_update name", () => {
		expect(createSelfUpdateTool("scribe").name).toBe("self_update");
	});
	...
	it("enforces the SOUL.md budget and writes nothing on overflow", async () => {
		const big = "x".repeat(SOUL_BUDGET + 100);
		const result = await run({ file: "SOUL", op: "replace", content: big });
		expect(result.details.applied).toBe(false);
		expect(firstText(result)).toMatch(/over the .*budget/);
		expect(readMemoryFile(getSoulPath("scribe"))).toBe("");
	});

	it("writes whole-file SOUL.md replacements", async () => {
		await run({ file: "SOUL", op: "replace", content: "I am Scribe.\nPurpose: meeting memory." });
		expect(readMemoryFile(getSoulPath("scribe"))).toBe("I am Scribe.\nPurpose: meeting memory.\n");
	});
